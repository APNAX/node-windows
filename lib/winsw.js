...
console.log({loc: 'winsw.js ~line 77', xml, config})
    ...
    if (config.logOnAs) {
      ...
      xml.push({serviceaccount: serviceaccount});
    }

    // Optionally set the start mode
    if (config.startMode) {
      xml.push({startmode: config.startMode});
    }

    // Optionally set delayed start mode
    if (config.delayedAutoStart === true) {
      xml.push({delayedAutoStart: true});
    }
